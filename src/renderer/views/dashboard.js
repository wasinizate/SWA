...
import { formatMoney, loadingHtml } from '../helpers.js';
...
export function renderDashboardView(container, { navigate }) {
  ...
  async function init() {
    const [people, openOrderCount, dueSummary, totalsAll, slated, note] = await Promise.all([
    ...
    renderStats({ clientCount: people.length, openOrderCount, totalsAll, slated });
    renderDue(dueSummary);
    container.querySelector('#dashboard-note').value = note ?? '';
  }

  function renderStats({ clientCount, openOrderCount, totalsAll, slated }) {
